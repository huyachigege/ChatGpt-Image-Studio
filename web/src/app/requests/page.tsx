              ...
              {items.map((item) => (
                <div key={item.id} className="rounded-2xl border border-stone-200/80 bg-stone-50/60 p-4">
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    ...
                    <Badge variant="secondary" className="rounded-md bg-stone-100 text-stone-700">{item.imageMode || "studio"}</Badge>
                    <Badge variant={item.direction === "cpa" ? "info" : "success"} className="rounded-md px-2 py-1">{item.direction === "cpa" ? "CPA" : "官方"}</Badge>
                  </div>
                  ...
                  <div className="mt-3 grid grid-cols-1 gap-3 text-sm text-stone-600 sm:grid-cols-2">
                    ...
                    <InfoBox title="接口" main={item.endpoint || "—"} breakAll />
                    <InfoBox title="参数" main={item.size || "—"} sub={item.quality ? `quality: ${item.quality}` : "quality: —"} />
                    <InfoBox title="提示词" main={promptText(item)} className="sm:col-span-2" breakAll />
                    ...
                    <InfoBox title="模型" main={item.requestedModel || "—"} sub={item.upstreamModel || "—"} />
                    <InfoBox title="错误" main={item.error || "—"} className="sm:col-span-2" breakAll />
                  </div>
                </div>
              ))}
            </div>
                    ...
                    <tbody>
                      {items.map((item) => (
                        <tr key={item.id} className="border-b border-stone-100/80 text-sm text-stone-600 hover:bg-stone-50/70">
                          ...
                          <td className="px-4 py-3 whitespace-nowrap"><Badge variant="secondary" className="rounded-md bg-stone-100 text-stone-700">{item.imageMode || "studio"}</Badge></td>
                          <td className="px-4 py-3 whitespace-nowrap"><Badge variant={item.direction === "cpa" ? "info" : "success"} className="rounded-md px-2 py-1">{item.direction === "cpa" ? "CPA" : "官方"}</Badge></td>
                          <td className="px-4 py-3 whitespace-nowrap">
                            <div>{item.route || "—"}</div>
                          ...
                          <td className="px-4 py-3">
                            <div className="max-w-[280px] truncate text-xs text-stone-500" title={item.prompt || ""}>{promptText(item)}</div>
                          </td>
                          <td className="px-4 py-3 whitespace-nowrap">
                          ...
                          <td className="px-4 py-3"><div className="max-w-[320px] truncate text-xs text-stone-500" title={item.error || ""}>{item.error || "—"}</div></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
