...
export function RuntimeSection({ config, setSection }: RuntimeSectionProps) {
  return (
    <ConfigSection
      title="基础运行配置"
      description="后端服务监听地址、端口、上传大小、超时和账号刷新等常用配置。修改监听地址或端口后，需要重启程序才会生效。"
    >
      <Field
        label="当前版本"
            ...
            items={[
              {
                ...
                body: (
                  <>
                    由后端运行时返回；发布版会在构建时写入版本号、提交号和构建时间。
                  </>
                ),
              },
              {
                ...
                body: (
                  <>
                    排查“用户下载的到底是哪一版”时，优先看这里，不要只看前端角落的展示版本。
                  </>
                ),
              },
            ]}
            ...
            items={[
              {
                ...
                body: (
                  <>
                    <code>0.0.0.0</code> 表示局域网可访问，
                    <code>127.0.0.1</code> 表示只允许本机访问。
                  </>
                ),
              },
              {
                ...
                body: (
                  <>
                    只是自己本机使用时填 `127.0.0.1` 更收敛；需要局域网访问再用
                    `0.0.0.0`。
                  </>
                ),
              },
      ...
      >
        <Input
          ...
          onChange={(event) =>
            setSection("server", { ...config.server, host: event.target.value })
          }
          ...
          <TooltipDetails
            items={[
              ...
              },
              {
                ...
                body: (
                  <>
                    如果启动时报端口占用，最直接的处理就是改成一个空闲端口再重启。
                  </>
                ),
              },
            ]}
      ...
      <Field
        label="图片下载限速（KB/s）"
        hint="限制普通用户查看原图和下载原图的单连接速度；填 0 表示不限速，管理员不限速。"
      >
        <Input
          type="number"
          min={0}
          value={String(config.server.imageDownloadRateKbps)}
          onChange={(event) =>
            setSection("server", {
              ...config.server,
              imageDownloadRateKbps: Math.max(
                0,
      ...
      >
        <Input
          ...
          onChange={(event) =>
            setSection("app", { ...config.app, authKey: event.target.value })
          }
          className="h-11 rounded-2xl border-stone-200 bg-white shadow-none"
        />
      </Field>
            ...
            items={[
              {
                title: "格式",
                body: (
                  <>支持多个 key，用英文逗号分隔，例如 `key-a,key-b,key-c`。</>
                ),
              },
              {
                title: "作用",
                body: (
                  <>
                    调用当前项目对外暴露的图片 API 时，会校验请求头里的 Bearer
                    token 是否命中这里。
                  </>
                ),
              },
      ...
      >
        <Input
          ...
          onChange={(event) =>
            setSection("app", { ...config.app, apiKey: event.target.value })
          }
          className="h-11 rounded-2xl border-stone-200 bg-white shadow-none"
        />
      </Field>
            ...
            items={[
              {
                ...
                body: (
                  <>
                    限制单张上传图片的最大体积，超出后会在进入模型前就被后端拦掉。
                  </>
                ),
              },
              {
                title: "建议值",
                body: (
                  <>通常 `20` 到 `100` MB 足够；上传原图较大时可适当调高。</>
                ),
              },
              {
                title: "太小的表现",
                body: (
                  <>
                    编辑或参考图上传会直接失败，即使图片本身格式没问题也会被拒绝。
                  </>
                ),
              },
            ]}
            ...
            items={[
              {
                ...
                body: (
                  <>
                    控制普通官方 HTTP 请求的超时，例如非流式接口和常规下载请求。
                  </>
                ),
              },
              {
              ...
              },
              {
                ...
                body: (
                  <>
                    这个值不是长时间图片生成等待的总超时，长等待主要看下面的 SSE
                    超时。
                  </>
                ),
              },
            ...
            items={[
              ...
              },
              {
                ...
                body: (
                  <>
                    默认是 `600`
                    秒；通常应明显大于普通请求超时，长图或排队场景建议保留较大值。
                  </>
                ),
              },
              {
                ...
                body: (
                  <>
                    长图或高峰期排队时更容易出现“timed out waiting for async
                    image generation”。
                  </>
                ),
              },
            ...
            items={[
              {
                ...
                body: (
                  <>
                    只影响 `legacy` 图像链路，控制后端多久去问一次任务是否完成。
                  </>
                ),
              },
              {
                ...
                body: (
                  <>
                    结果会更快被发现，但请求次数会更多，更容易增加噪音和上游压力。
                  </>
                ),
              },
              {
          ...
          <TooltipDetails
            items={[
              ...
              },
              {
                ...
                body: (
                  <>
                    任务其实还在上游处理中，但本地会先报超时，看起来像“图还没回来就失败”。
                  </>
                ),
              },
            ]}
            ...
            items={[
              {
                ...
                body: (
                  <>
                    当后端暂时拿不到真实图片额度时，会先用这个数值做本地初始显示和兜底。
                  </>
                ),
              },
              {
              ...
              },
              {
                ...
                body: (
                  <>
                    填得再大也不会真的让账号有更多图片额度，只会影响本地展示与初始判断。
                  </>
                ),
              },
            ]}
            ...
            items={[
              {
                title: "作用",
                body: (
                  <>控制批量刷新账号信息时同时开多少个 worker 去请求上游。</>
                ),
              },
              {
                title: "建议值",
            ...
            items={[
              {
                ...
                body: (
                  <>
                    会优先尝试去远端拿真实额度与状态，显示更准，但刷新耗时通常更长。
                  </>
                ),
              },
              {
      ...
      <ToggleField
            ...
            items={[
              {
                ...
                body: (
                  <>
                    控制台和日志里会输出更细的请求方向、路由、账号类型等信息，排障更方便。
                  </>
                ),
              },
              {
        ...
        onCheckedChange={(checked) =>
          setSection("log", { ...config.log, logAllRequests: checked })
        }
      />
    </ConfigSection>
  );
